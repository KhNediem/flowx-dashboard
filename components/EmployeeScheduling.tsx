...
export function EmployeeScheduling() {
  ...
  async function fetchData() {
    try {
      ...
      const { data: fetchedPredictions, error: predictionsError } =
        await supabase.from("schedule_predictions").select("*");

      if (predictionsError) {
        console.error("Error fetching schedule predictions:", predictionsError);
      } else {
        setPredictionSchedules(fetchedPredictions ?? []);
      }
    } catch (error) {
  ...
  const filteredHoursData = combinedData.filter((item) => {
    ...
    try {
      ...
      const fromDate = hoursDateRange?.from
        ? new Date(hoursDateRange.from)
        : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="h-[400px] border rounded-lg p-4">
        {selectedChart === "gantt" && (
          <GanttChart data={filteredGanttData} dateRange={chartDateRange} />
        )}
